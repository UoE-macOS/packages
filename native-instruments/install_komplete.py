...
FULLPRODS = ["00b224af-b357-4dfe-9929-414bbdf97d6f",
            ...
            try:
                ...
                for art in latest:
                    ...
                    for (candidate, version) in files:
                        if not is_installed(candidate, version) and not download_only:
...
def install_native_access():
    fetch(NATIVE_ACCESS_URL, dest='_downloads/native-access.dmg')
    path, pkgs = attach_image('_downloads/native-access.dmg')
