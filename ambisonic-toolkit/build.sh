...
# Create the directory structure we will install to
...
# Copy content over to user template
...
# But copy the data to /Library/Application Support
cp -R "${build}/ATK for Reaper/Copy content to Data/ATK/" "${build}/pkgroot/${ATK_DATA}/"

# And symlink it into the user template
ln -s "/${ATK_DATA}" "${build}/pkgroot/${USER_TEMPLATE}/Data/ATK"

# Then build the package
pkgbuild --root ${build}/pkgroot --id net.ambisonictoolkit.atk-reaper --version "${VERSION}" ${build}/AmbisonicToolkitForREAPER-${VERSION}-${RELEASE}.pkg
